...
def draw_button(frame, idx, lit=False):
    ...
    if lit:
        overlay = frame.copy()
        cv2.circle(overlay, (x, y), btn_radius + 14, (255,255,255), 8)
        frame[:] = cv2.addWeighted(overlay, 0.14, frame, 0.86, 0)

def draw_pulse(frame, pulse):
    """Pulse: {lane, created, ttl, color}"""
    ...
    overlay = frame.copy()
    cv2.circle(overlay, (x, y), radius, pulse['color'], 10)
    frame[:] = cv2.addWeighted(overlay, 0.14 * alpha + 0.02, frame, 1 - (0.14 * alpha + 0.02), 0)
    return True
...
while True:
    ...
    if state == 'menu':
        draw_text_center(frame, "SIMON DICE (OpenCV)", 100, 1.3)
        draw_text_center(frame, "[F] Facil (1 color inicio, 10s/por color)", 220, 0.8)
        draw_text_center(frame, "[N] Normal (2 colores inicio, 7s/por color)", 270, 0.8)
        draw_text_center(frame, "[D] Dificil (3 colores inicio, 5s/por color)", 320, 0.8)
        draw_text_center(frame, "[ESPACIO] Alternar Modo Infinito: " + ("ON" if modo_infinito else "OFF"), 380, 0.8)
        draw_text_center(frame, "Pulsa la tecla correspondiente para empezar", 450, 0.7)
        thumb = cv2.resize(cam, (260, 180))
        tx, ty = WIDTH - thumb.shape[1] - 20, 20
        frame[ty:ty+thumb.shape[0], tx:tx+thumb.shape[1]] = thumb
        if detected_color:
            cv2.putText(frame, f"Detectado: {detected_color.upper()}", (tx+6, ty+thumb.shape[0]-8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color_map[detected_color], 2)
        else:
            cv2.putText(frame, "Detectado: NINGUNO", (tx+6, ty+thumb.shape[0]-8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200,200,200), 2)
    ...
    if state == 'player_turn':
        ...
        if expected_idx < len(sequence):
            ...
            if time_left < 0:
                pulses.append({'lane': sequence[expected_idx], 'created': time.time(), 'ttl': 0.9, 'color': pulse_fail})
        ...
        if input_lane is not None:
            ...
            if input_lane == correct_lane:
                pulses.append({'lane': input_lane, 'created': time.time(), 'ttl': 0.7, 'color': pulse_ok})
                expected_idx += 1
                player_turn_start = time.time()
                if expected_idx >= len(sequence):
                    for i in range(4):
                        pulses.append({'lane': i, 'created': time.time(), 'ttl': 0.8, 'color': (80,180,80)})
                ...
                pulses.append({'lane': input_lane, 'created': time.time(), 'ttl': 1.0, 'color': pulse_fail})
